(function (L) {
    ...
    _this = L.Balancer = {
        ...
        initEvents: function () {
            ...
            _this.initAddBackupUpstreamChangeEvent();   //新增backup节点下拉框改变事件
            _this.initAddHostChangeEvent();             //新增输入host节点下拉框改变时间
            _this.initEurekaChangeEvent();              //新增输入eureka节点下拉框改变时间
        ...
        //新增backup节点选择数值改变事件
        initAddBackupUpstreamChangeEvent: function () {
            $(document).on("change", '#backup-upstream-enable', function () {
                var value = $(this).val();
                if (value == "1") {
                    $("#backup-upstream-add-input-area").show();
                    _this.reloadBackUpSelect();
                } else {
                    $("#backup-upstream-add-input-area").hide();
                }
            });
        },
        //新增backup节点选择数值改变事件
        initAddHostChangeEvent: function () {
            $(document).on("change", '#input-style', function () {
                var value = $(this).val();
                if (value == "2") {
                    $("#eureka-area").show();
                    $("#eureka-target-area").show();
                    $("#target-area").hide();
                    _this.reloadEurekaSelect();
                } else {
                    $("#target-area").show();
                    $("#eureka-area").hide();
                    $("#eureka-target-area").hide();
                }
            });
        },
        //新增eureka下拉框改变时间
        initEurekaChangeEvent: function () {
            $(document).on("change", '#rule-eureka-name', function () {
                _this.reloadEurekaTarget();
            });
        },
        reloadEurekaTarget: function (eureka_name, target) {
            $("#eureka-target-name").html("");
            var value = $("#rule-eureka-name").val();
            if (!value || value == "") {
                value = eureka_name;
            }
            $.ajax({
                url: '/balancer/eureka/get/apps',
                type: 'get',
                cache: false,
                data: {
                    name: value
                ...
                success: function (result) {
                    if (result.success) {
                        if (result.data && result.data.application && result.data.application.instance) {
                            var instances = result.data.application.instance;
                            for (var i = 0; i < instances.length; i++) {
                                var instance = instances[i];
                                if (instance.status == "UP") {
                                    if (target != null && target == instance.instanceId) {
                                        $("#eureka-target-name").append("<option value=\"" + instance.instanceId + "\" selected>" + instance.instanceId + "</option>");
                                    } else {
                                        $("#eureka-target-name").append("<option value=\"" + instance.instanceId + "\" >" + instance.instanceId + "</option>");
                                    }
                                }
                            }
                        }
                    } else {
                        L.Common.showErrorTip("错误提示", "查询eureka app配置请求发生错误" + result.data);
                    }
                },
                error: function () {
                    L.Common.showErrorTip("提示", "查询eureka app配置请求发生异常");
                }
            });
        },
        reloadEurekaSelect: function (eureka_name, target) {
            $("#rule-eureka-name").html("");
            //加载upstream数据
            $.ajax({
                url: '/balancer/eureka/apps',
                type: 'get',
                cache: false,
                data: {},
                //async: false,
                ...
                success: function (result) {
                    if (result.success) {
                        if (result.data && result.data.applications && result.data.applications.application) {
                            var isFirst = true;
                            var first_eureka_name = "";
                            var applications = result.data.applications.application;
                            for (var i = 0; i < applications.length; i++) {
                                ...
                                if (status) {
                                    ...
                                    if (name.indexOf("EUREKA") == -1 && name.indexOf("SORT") == -1 && name.indexOf("CONFIG") == -1 && name.indexOf("SPRING") == -1) {
                                        if (isFirst) {
                                            first_eureka_name = name;
                                            isFirst = false;
                                        }
                                        //设置赋值
                                        if (eureka_name != null && eureka_name == name) {
                                            $("#rule-eureka-name").append("<option value=\"" + name + "\" selected>" + name + "</option>");
                                        } else {
                                            $("#rule-eureka-name").append("<option value=\"" + name + "\" >" + name + "</option>");
                                        }
                                    }
                                } else {
                            ...
                            if (target != null && eureka_name != null) {
                                _this.reloadEurekaTarget(first_eureka_name, target);
                            } else {
                                _this.reloadEurekaTarget(null, first_eureka_name);
                            }
                        }
                    } else {
                        L.Common.showErrorTip("错误提示", "查询eureka配置请求发生错误" + result.data);
                    }
                },
                error: function () {
                    L.Common.showErrorTip("提示", "查询eureka配置请求发生异常");
                }
            });
        },

        reloadBackUpSelect: function (backup) {
            $("#backup-upstream-selector").html("");
            //加载upstream数据
            $.ajax({
                            ...
                            if (to_render_ids) {
                                for (var i = 0; i < to_render_ids.length; i++) {
                                    if (selectors[to_render_ids[i]]) {
                                        var selector = selectors[to_render_ids[i]];
                                        if (backup != null && backup == selector.name) {
                                            $("#backup-upstream-selector").append("<option value=\"" + selector.name + "\" selected>" + selector.name + "</option>");
                                        } else {
                                            if (selector.enable) {
                                                $("#backup-upstream-selector").append("<option value=\"" + selector.name + "\">" + selector.name + "</option>");
                                            }
                                        }
                                    }
                                }
        ...
        initUpstreamEditDialog: function (context) {
            $(document).on("click", ".edit-selector-btn", function (e) {
                ...
                if (selector.backup_enable) {
                    _this.reloadBackUpSelect(selector.backup);
                }
                ...
                var d = dialog({
                    ...
                    button: [{
                        ...
                        callback: function () {
                            ...
                            if (result.success == true) {
                                _this.addHost(selector_id, result.data, context);
                            } else {
                                L.Common.showErrorTip("错误提示", result.data);
                                return false;
                            }
        ...
        //新增host
        addHost: function (selector_id, data, context) {
            $.ajax({
        ...
        initHostEditDialog: function (context) {
            $(document).on("click", ".edit-btn", function () {
                ...
                if (rule.input_style == 2) {
                    _this.reloadEurekaSelect(rule.eureka_name, rule.target);
                }
                var html = juicer(tpl, {
                    r: rule
                });
        ...
        buildUpstream: function () {
            ...
            var backup_enable = $("#backup-upstream-enable").val();
            if (!backup_enable) {
                // keep the default
            } else {
                result.data.backup_enable = parseInt(backup_enable) == 1;
            }

            //启用backup节点才需要对backup值进行赋值和校验
            if (result.data.backup_enable) {
                var backup = $("#backup-upstream-selector").val();
                if (!backup) {
                    result.success = false;
                    result.data = "backup upstream不能为空";
                    return result;
                } else {
                    result.data.backup = backup;
                }
            }
            result.success = true;
            return result;
        ...
        buildHost: function () {
            ...
            var input_style = $("#input-style").val();
            if (!input_style) {
                result.data = "输入方式不能为空";
                return result;
            }
            result.data.input_style = parseInt(input_style);

            //1为手动输入 2为eureka
            //手动输入target字段不能为空
            if (result.data.input_style == 1) {
                ...
                var eureka_name = $("#rule-eureka-name").val();
                if (!eureka_name) {
                    result.data = "eureka service name不能为空";
                    return result;
                }
                result.data.eureka_name = eureka_name;

                var target_name = $("#eureka-target-name").val();
                if (!target_name) {
                    result.data = "target不能为空";
                    return result;
                }
                result.data.target = target_name;
            }
